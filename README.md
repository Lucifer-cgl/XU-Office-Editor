...
`dist/` 是完整的运行时包，不依赖仓库外的源码：

```text
dist/
├─ start.cmd       # 双击启动
├─ start.ps1       # Windows 内置 PowerShell/.NET 本地服务
├─ index.html
├─ src/            # 工作台界面代码
├─ assets/         # Office WebAssembly、字体和运行资源
└─ sw.js
```

访客只需要下载并解压 `dist/`，双击 `start.cmd` 即可运行；在 XU 中则选择这个 `dist/` 文件夹完成深度嵌入。仓库中的其他源码主要用于维护和重新生成新的 dist 版本，不是访客运行的依赖。
